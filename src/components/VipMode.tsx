...
const DraggableNode = ({ 
  ...
  return (
    ...
    >
      ...
      >
        {isEditMode && (
          <div className="absolute -top-4 left-1/2 -translate-x-1/2 text-yellow-400 hover:text-yellow-300 transition-colors bg-black/40 rounded-full p-1.5 backdrop-blur-sm cursor-grab active:cursor-grabbing z-50 shadow-lg shadow-yellow-500/20 border border-yellow-500/30">
...
export const VipMode = ({ 
  ...
  const mapBgColor = settings.map_bg_color || '#0f0c29'; // Darker, more purple base
  const mapBorderColor = mapColor || settings.map_border_color || '#facc15'; // Gold/Yellow
  const mapFillColor = mapColor || settings.map_fill_color || 'rgba(250, 204, 21, 0.05)';
  ...
  const colorPalette = ['#facc15', '#f43f5e', '#8b5cf6', '#10b981', '#3b82f6', '#ffffff'];

  return (
    <div className="relative w-full h-screen overflow-hidden text-white bg-slate-950"
         style={{ 
           backgroundColor: mapBgColor,
           background: `linear-gradient(to bottom right, ${mapBgColor}, #24243e)`,
           fontFamily: mapFontFamily,
           '--MainColor': settings.primary_color || '#facc15' 
      ...
      {isLoggedIn && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] flex flex-wrap gap-2 w-full max-w-md justify-center px-4" dir="rtl">
          ...
          <button 
            ...
            className="px-4 md:px-6 py-2.5 rounded-full font-bold text-slate-900 bg-yellow-400/90 hover:bg-yellow-300/90 shadow-lg shadow-yellow-500/30 transition-all flex-1 min-w-[100px] text-xs md:text-sm whitespace-nowrap backdrop-blur-md border border-yellow-400/50"
          >
            إضافة مربع نص
          </button>
        </div>
                ...
                >
                  ...
                  >
                    ...
                    <EditableText 
                        ...
                        defaultSize={14} 
                        className="absolute -bottom-8 font-bold text-white text-center whitespace-nowrap drop-shadow-[0_0_4px_rgba(0,0,0,1)] bg-black/40 px-3 py-0.5 rounded-full border border-white/10 backdrop-blur-sm transition-all duration-300 z-20 opacity-90 group-hover:opacity-100 group-hover:text-yellow-200"
                        config={config}
                        updateConfig={updateConfig}
                        isEditMode={isEditMode}
                    />
                ...
                return (
                  <div key={uni.id} className="bg-white/5 p-4 rounded-2xl border border-white/5 hover:border-white/20 transition-colors">
                    ...
                    <div className="flex gap-3 mb-3" dir="ltr">
                      <div className="flex-1 relative">
                        ...
                        <input 
                          ...
                          className="w-full bg-black/50 border border-white/10 rounded-lg pl-6 pr-2 py-1.5 text-white text-sm font-mono focus:border-yellow-500 outline-none transition-colors" 
                        />
                      </div>
                      <div className="flex-1 relative">
                    ...
                    <button 
                      ...
                      className="w-full flex items-center justify-center gap-2 bg-yellow-500/10 hover:bg-yellow-500/30 text-yellow-300 text-xs py-2 rounded-lg transition-colors border border-yellow-500/20"
                    >
                      <Crosshair size={14} />
                      إحضار للمنتصف
                    </button>
        ...
        {isEditMode && (
          ...
          >
            <button
              ...
              className="flex items-center gap-2 bg-yellow-500/20 hover:bg-yellow-500/40 text-yellow-300 px-4 py-2 rounded-full font-bold transition-all border border-yellow-500/30"
            >
              <Plus size={18} />
              إضافة نص حر
            </button>
      ...
      <div className="absolute top-6 left-6 z-50 flex items-center gap-4">
        <button className="p-3 bg-gradient-to-r from-purple-600 to-pink-600 backdrop-blur-md rounded-full hover:from-purple-500 hover:to-pink-500 transition-all border border-white/20 shadow-[0_0_20px_rgba(168,85,247,0.4)] flex items-center gap-2">
          <span className="text-white font-bold text-sm px-2">إعلان</span>
        </button>
      </div>
      ...
      <AnimatePresence>
        {selectedUni && viewMode === 'summary' && (
          ...
          >
            <div className="bg-[#020617]/60 backdrop-blur-2xl border border-yellow-500/30 p-6 rounded-3xl shadow-[0_0_30px_rgba(234,179,8,0.2)] w-full pointer-events-auto relative text-center" dir="rtl">
              ...
              {isLoggedIn && (
                <button 
                  ...
                  className={cn(
                    ...
                    isDescriptionEditing ? "bg-yellow-500 hover:bg-yellow-400 text-slate-900" : "bg-white/10 hover:bg-white/20"
                  )}
                >
                  <Edit2 size={16} />
                </button>
              ...
              <div className="mb-4 flex justify-center mt-2">
                {selectedUni.logo_url ? (
                  <img src={selectedUni.logo_url} alt={selectedUni.name} className="w-20 h-20 object-contain bg-white/5 rounded-2xl p-2 shadow-[0_0_15px_rgba(234,179,8,0.2)] border border-yellow-500/20" referrerPolicy="no-referrer" />
                ) : (
                  <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-yellow-500/20 to-orange-600/20 border border-yellow-500/30 flex items-center justify-center text-yellow-300 font-bold text-3xl shadow-[0_0_15px_rgba(234,179,8,0.2)]">
              ...
              {isDescriptionEditing ? (
                <div className="space-y-3 mb-6">
                  <div>
                    <label className="block text-[10px] font-black text-yellow-400 uppercase mb-1 text-right">تغيير الشعار</label>
                    <label className="flex items-center justify-center gap-2 p-2 bg-white/5 border border-dashed border-yellow-500/30 rounded-xl cursor-pointer hover:bg-white/10 transition-all text-xs text-white/70">
                      <Upload size={14} />
                  ...
                  <div>
                    <div className="flex justify-between mb-1">
                      <label className="text-[10px] font-black text-yellow-400 uppercase text-right">حجم النص</label>
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-yellow-400 uppercase mb-1 text-right">الوصف</label>
                    <textarea
                      className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white leading-relaxed outline-none focus:border-yellow-500 min-h-[80px] text-right resize-none"
                  ...
                  <button 
                    ...
                    className="w-full py-2 bg-yellow-500 hover:bg-yellow-400 text-slate-900 rounded-xl text-sm font-bold transition-colors shadow-[0_0_15px_rgba(234,179,8,0.3)]"
                  >
                    حفظ التعديلات
                  </button>
                </div>
              ...
              <button
                ...
                className="w-full py-3 bg-gradient-to-r from-yellow-600 to-yellow-400 hover:from-yellow-500 hover:to-yellow-300 text-slate-900 rounded-xl font-black text-sm shadow-[0_0_20px_rgba(234,179,8,0.3)] transition-all transform hover:scale-[1.02] flex items-center justify-center gap-2"
              >
                ...
                <AnimatePresence mode="wait">
                  ...
                  {modalStep === 'description' && (
                    ...
                    >
                        ...
                        {isEditMode && handleUpdateUni ? (
                          <div className="space-y-4">
                            <div className="flex items-center gap-4 justify-center mb-4">
                              <label className="text-sm font-bold text-yellow-400">حجم النص:</label>
                              <input 
                                type="range" 
                                min="14" max="40" 
                            ...
                            <textarea
                              className="w-full bg-white/5 border border-white/10 rounded-2xl p-6 text-white leading-relaxed outline-none focus:border-yellow-500 min-h-[200px] text-center"
                      ...
                      <button
                        ...
                        className="group relative px-8 py-4 bg-yellow-500 hover:bg-yellow-400 text-slate-900 rounded-2xl font-black text-lg shadow-[0_0_20px_rgba(234,179,8,0.3)] transition-all overflow-hidden"
                      >
                    ...
                    >
                      <div className="mb-6 relative">
                        ...
                        <input 
                          ...
                          className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pr-12 pl-4 text-white placeholder:text-white/40 focus:border-yellow-500 outline-none transition-all"
                        />
                      </div>

                      {isEditMode && (
                        <div className="mb-6 bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-4 justify-center">
                          <label className="text-sm font-bold text-yellow-400">حجم نص المواد:</label>
                          <input 
                            type="range" 
                            min="12" max="30" 
                          ...
                          {uniSubjects
                            ...
                            .map((sub, idx) => (
                              <motion.div
                                ...
                                className="bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl p-5 cursor-pointer group transition-all hover:scale-[1.02] hover:shadow-lg hover:shadow-yellow-500/10"
                              >
                                <div className="flex justify-between items-start mb-3">
                                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-yellow-500/20 to-orange-500/20 flex items-center justify-center text-yellow-400 group-hover:text-white transition-colors">
                                    <FileText size={20} />
                                  </div>
                  ...
                  {modalStep === 'plans' && selectedSubject && (
                    ...
                    >
                      ...
                      {isEditMode && (
                        <div className="mb-6 bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-4 justify-center">
                          <label className="text-sm font-bold text-yellow-400">حجم نص الخطط:</label>
                          <input 
                            type="range" 
                            min="12" max="30" 
                      ...
                      <div className="grid grid-cols-1 gap-4 pb-20">
                        {(uniPlans[selectedSubject.id] || []).map((plan, idx) => (
                          ...
                          >
                            <div>
                              <h4 className="font-bold text-white mb-2 group-hover:text-yellow-400 transition-colors" style={{ fontSize: `${config[`plans_text_size`] || 20}px` }}>{plan.name}</h4>
                              <p className="text-white/60 line-clamp-2" style={{ fontSize: `${(config[`plans_text_size`] || 20) * 0.7}px` }}>{plan.description}</p>
                            </div>
                            <div className="flex items-center gap-4">
                              <span className="text-yellow-400 font-mono font-bold text-lg">{plan.price} KWD</span>
                              <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-yellow-500 group-hover:text-slate-900 transition-all">
                                <ChevronLeft size={20} />
                              </div>
                            </div>
                          </motion.div>
                    ...
                    >
                      ...
                      {isEditMode && (
                        <div className="mb-6 bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-4 justify-center shrink-0">
                          <label className="text-sm font-bold text-yellow-400">حجم نص التفاصيل:</label>
                          <input 
                            type="range" 
                            min="14" max="40" 
                      ...
                      <div className="flex-1 overflow-y-auto custom-scrollbar pb-24 space-y-6">
                        ...
                        {selectedPlan.outputs && selectedPlan.outputs.trim().length > 0 && (
                          <div className="bg-white/5 rounded-3xl p-6 border border-white/10">
                            <h3 className="font-black text-white mb-4" style={{ fontSize: `${(config[`plan_details_text_size`] || 18) * 1.2}px` }}>
                              {selectedPlan.outputs_label || 'المخرجات'}
                            </h3>
                            <ul className="space-y-3">
                              {selectedPlan.outputs.split('\n').filter(line => line.trim().length > 0).map((output, idx) => (
                                <li key={idx} className="flex items-start gap-3 text-white/80" style={{ fontSize: `${config[`plan_details_text_size`] || 18}px` }}>
                                  <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-yellow-500 shrink-0" />
                                  <span className="leading-relaxed">{output.trim().replace(/^-/, '').trim()}</span>
                                </li>
                              ))}
                            </ul>
                          </div>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-6 bg-[#0f172a]/95 backdrop-blur-xl border-t border-white/10 flex items-center justify-between gap-4 z-20">
                        ...
                        <button
                          ...
                          className="flex-1 bg-yellow-500 hover:bg-yellow-400 text-slate-900 py-4 rounded-2xl font-black text-lg shadow-lg shadow-yellow-500/20 flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98]"
                        >
                          <EditableText settingKey="map_btn_confirm_plan" defaultText="تأكيد الخطة" settings={settings} isLoggedIn={isLoggedIn} />
                        </button>
                        <button
                          onClick={() => {
                            const message = `${settings.whatsapp_prefix || 'مرحباً، أود الاستفسار عن'}\n\n*الخطة:* ${selectedPlan.name}\n*المادة:* ${selectedSubject?.name}\n*الجامعة:* ${selectedUni?.name}`;
                            window.open(`https://wa.me/${settings.whatsapp_number}?text=${encodeURIComponent(message)}`, '_blank');
                          }}
                          className="bg-[#25D366] hover:bg-[#128C7E] text-white p-4 rounded-2xl shadow-lg shadow-[#25D366]/20 transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center"
                        >
            ...
            >
              ...
              <div className="p-6 border-t border-white/10 bg-white/5 flex justify-end gap-4">
                ...
                <button 
                  ...
                  className="px-8 py-3 rounded-xl font-bold text-slate-900 bg-yellow-500 hover:bg-yellow-400 shadow-lg shadow-yellow-500/20 transition-all"
                >
              ...
              <div className="p-6 flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                      {selectedPlan.outputs?.split('\n').filter(line => line.trim().length > 0).map((output, idx) => (
                        <label key={idx} className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/10 cursor-pointer hover:bg-white/10 transition-colors">
                          <input 
                            type="checkbox" 
                            className="mt-1 w-4 h-4 rounded border-white/20 bg-black/50 text-yellow-500 focus:ring-yellow-500 focus:ring-offset-slate-900"
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors"
                  />
                </div>
              </div>
